...
public interface CallDetailRepository extends JpaRepository<CallDetail, Long> {
    ...
    @Query("""
        ...
        WHERE (:name IS NULL OR c.empId IN 
              (SELECT e.empId FROM Employee e WHERE LOWER(e.empName) LIKE LOWER(CONCAT('%', :name, '%'))))
        AND (:team IS NULL OR LOWER(c.team) LIKE LOWER(CONCAT('%', :team, '%')))
